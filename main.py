...
def setup_and_extract_data(cred_path, table_index = 2): # Step 1: Specify the correct file path
...
def three_etl_store_details():
    ...
    stores_df, table_name, csv_filename = data_extractor.retrieve_stores_data(retrieve_a_store_endpoint, headers, number_of_stores)  # retrieve data for all stores and save in a Pandas df
...
def six_etl_date_events():
    ...
    raw_csv_filename = os.path.join(cleaned_csv_folder_path, f"{table_name}_data_cleaned.csv")  # Save the cleaned DataFrame as a CSV file in the specified folder
...
if __name__ == "__main__":
    ...
    print("######################################## 1. etl_of_leagacy_users ########################################")
    one_etl_leagacy_users()

    print("######################################## 2. etl_of_card_details ########################################")
    two_etl_card_details()

    print("######################################## 3. etl_of_store_details ########################################")
    three_etl_store_details()

    print("######################################## 4. etl_of_product_details ########################################")
    four_etl_product_details()

    print("######################################## 5. etl_of_orders_details ########################################")
    five_etl_orders_details()

    print("######################################## 6. etl_of_date_events ########################################")
    six_etl_date_events()
